/*-----------------------------------------------------------------------------
BuildAdventurer.js - Minimal Version
Female base body with sash dress options
-----------------------------------------------------------------------------*/
...
const SASH_DRESSES = [
  { key: "red", label: "Red Sash Dress" },
  { key: "blue", label: "Blue Sash Dress" },
  { key: "green", label: "Green Sash Dress" },
  { key: "purple", label: "Purple Sash Dress" },
  { key: "black", label: "Black Sash Dress" },
];

/* -----------------------------------------------------------------------------
 SpritePreview: shows stacked sprite layers (base + dress)
-----------------------------------------------------------------------------*/
function SpritePreview({ sashDress, name }) {
  // Map dress keys to their actual filenames
  const dressFileNames = {
    red: "red",
    blue: "blue",
    green: "green",
    purple: "purple",
    black: "black-w-black-lining",  // Custom filename for black
  };
  ...
  const layers = useMemo(
    () => [
      {
        id: "base",
        url: `/sprites/avatar/base_female.png`,
        // fallback: "👩",
      },
      {
        id: "dress",
        url: `/sprites/avatar/sash_dress_${dressFileNames[sashDress]}.png`,
        // fallback: "👗",
      },
    ],
    [sashDress, dressFileNames]
  );
...
export default function BuildAdventurer() {
  ...
  const [sashDress, setSashDress] = useState("black");
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Sash Dress selection */}
            <div>
              <div style={{ fontWeight: 700, marginBottom: 6 }}>
                Sash Dress
              </div>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {SASH_DRESSES.map((dress) => (
                  <label
                    key={dress.key}
                    className="chip"
                    style={{
                      padding: "8px 12px",
                      background: sashDress === dress.key ? "#2f241a" : "#6e3f1c",
                      opacity: 0.95,
                      cursor: "pointer",
                      color: "#fff7e8",
                    }}
                  >
                    <input
                      ...
                      name="sashDress"
                      value={dress.key}
                      checked={sashDress === dress.key}
                      onChange={() => setSashDress(dress.key)}
                      style={{ display: "none" }}
                    />
                    {dress.label}
                  </label>
          ...
          >
            <SpritePreview sashDress={sashDress} name={name} />
          </div>
        </div>
      </div>
    </main>
